...
wires as (
    select string_to_array(val, ',') as wire
...
compute as (
    ...
    SELECT x2
          ,y2
          ,x2 + CASE WHEN substring(wire[1], 1, 1) = 'R' THEN substring(wire[1],2,length(wire[1])-1)::int
                     WHEN substring(wire[1], 1, 1) = 'L' THEN -substring(wire[1],2,length(wire[1])-1)::int
                     ELSE 0
                END as x2
          ,y2 + CASE WHEN substring(wire[1], 1, 1) = 'U' THEN substring(wire[1],2,length(wire[1])-1)::int
                     WHEN substring(wire[1], 1, 1) = 'D' THEN -substring(wire[1],2,length(wire[1])-1)::int
                     ELSE 0
                END as y2
          ,CASE substring(wire[1], 1, 1) WHEN 'R' THEN 'H' WHEN 'L' THEN 'H' ELSE 'V' END as dir
          ,l+substring(wire[1],2,length(wire[1])-1)::int
          ,wire[2:]
          ,id
      FROM compute
     WHERE wire[1] IS NOT NULL
)
select cv.l+ch.l-abs(ch.x2-cv.x1)-abs(ch.y1-cv.y2) 
    from compute cv
    JOIN compute ch ON cv.x1>=least(ch.x1, ch.x2) AND cv.x1<=greatest(ch.x1, ch.x2) AND ch.y1>=least(cv.y1, cv.y2) AND ch.y2<=greatest(cv.y1, cv.y2) AND cv.id<>ch.id
   WHERE cv.dir='V' 
     AND ch.dir='H' 
     AND (cv.x1!=0 OR ch.y1!=0)
order by 1
   limit 1
-- select * from compute where id=1