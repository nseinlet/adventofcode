...
-- WITH machines as(VALUES('3,21,1008,21,8,20,1005,20,22,107,8,21,20,1006,20,31,1106,0,36,98,0,0,1002,21,125,20,4,20,1105,1,46,104,999,1105,1,46,1101,1000,1,20,4,20,1105,1,46,98,99')),
-- WITH machines as(VALUES('3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9')),
-- WITH machines as(VALUES('3,3,1105,-1,9,1101,0,0,12,4,12,99,1')),
machine as (
...
BEGIN
  WHILE (select data FROM machine_1202 WHERE op_id=pos AND machine=machine_id)::int4!=99
    LOOP
        ...
        IF operation!=4 AND operation!=5 AND operation!=6 THEN
            UPDATE machine_1202
        ...
        IF forced_operation>0 THEN
            pos := forced_operation;
        ELSE
...
-- SELECT run_machine(1,0);
SELECT run_machine(1,5);
-- SELECT run_machine(1,8);
-- SELECT run_machine(1,10);
