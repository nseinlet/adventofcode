...
no_lags as (
    SELECT seat,
           row_id,
           row_number() over(PARTITION BY row_id ORDER BY tmp_id) as seat_id
      FROM tmp_no_lsgs
)
SELECT l.seat,
       l.row_id::int,
       l.seat_id::int,
       abs(ll.thelag-l.seat_id) as lagl,
       abs(l.seat_id-lr.thelag) as lagr,
       abs(lf.thelag-l.row_id) as lagf,
       abs(l.row_id-lb.thelag) as lagb,
       abs(l.seat_id-lfl.thelag) as lagfl,
       abs(l.seat_id-lfr.thelag) as lagfr,
       abs(l.seat_id-lbl.thelag) as lagbl,
       abs(l.seat_id-lbr.thelag) as lagbr,
       length.l as lng
INTO TEMPORARY TABLE all_seats
  FROM no_lags l,length
LEFT JOIN LATERAL (SELECT MAX(nl.seat_id) as thelag FROM no_lags nl WHERE l.row_id = nl.row_id AND nl.seat_id < l.seat_id AND nl.seat!='.') ll ON TRUE -- lag left
LEFT JOIN LATERAL (SELECT MIN(nl.seat_id) as thelag FROM no_lags nl WHERE l.row_id = nl.row_id AND nl.seat_id > l.seat_id AND nl.seat!='.') lr ON TRUE -- lag right
LEFT JOIN LATERAL (SELECT MAX(nl.row_id)  as thelag FROM no_lags nl WHERE l.row_id > nl.row_id AND nl.seat_id = l.seat_id AND nl.seat!='.') lf ON TRUE -- lag front
LEFT JOIN LATERAL (SELECT MIN(nl.row_id)  as thelag FROM no_lags nl WHERE l.row_id < nl.row_id AND nl.seat_id = l.seat_id AND nl.seat!='.') lb ON TRUE -- lag back
LEFT JOIN LATERAL (SELECT MAX(nl.seat_id) as thelag FROM no_lags nl WHERE l.row_id > nl.row_id AND nl.seat_id < l.seat_id AND nl.seat!='.' AND abs(l.row_id-nl.row_id)=abs(l.seat_id-nl.seat_id) ) lfl ON TRUE -- lag front left
LEFT JOIN LATERAL (SELECT MIN(nl.seat_id) as thelag FROM no_lags nl WHERE l.row_id > nl.row_id AND nl.seat_id > l.seat_id AND nl.seat!='.' AND abs(l.row_id-nl.row_id)=abs(l.seat_id-nl.seat_id) ) lfr ON TRUE -- lag front right
LEFT JOIN LATERAL (SELECT MAX(nl.seat_id) as thelag FROM no_lags nl WHERE l.row_id < nl.row_id AND nl.seat_id < l.seat_id AND nl.seat!='.' AND abs(l.row_id-nl.row_id)=abs(l.seat_id-nl.seat_id) ) lbl ON TRUE -- lag back left
LEFT JOIN LATERAL (SELECT MIN(nl.seat_id) as thelag FROM no_lags nl WHERE l.row_id < nl.row_id AND nl.seat_id > l.seat_id AND nl.seat!='.' AND abs(l.row_id-nl.row_id)=abs(l.seat_id-nl.seat_id) ) lbr ON TRUE -- lag back right
  ;
...
WITH RECURSIVE seats AS(
    SELECT seat,
      ...
      -- computing is made one round over 2
      -- first we compute seat flip
      -- second we compute neigbours
     SELECT CASE
              WHEN s.counter%2=0 then s.seat
              WHEN s.seat='L' AND s.occupied=0 THEN '#'
              WHEN s.seat='#' AND s.occupied>=5 THEN 'L'
              ELSE s.seat
            END as seat,
            s.seat as old,
            s.row_id,
            ...
            s.lagbl,
            s.lagbr,
            s.counter+1 as counter,
            CASE WHEN (s.seat='L' OR s.seat='#') then
             (CASE WHEN LAG(seat,((s.lng*s.lagbl)+s.lagbl)::int) OVER(ORDER BY row_id DESC, seat_id     )='#' THEN 1 ELSE 0 END)
            +(CASE WHEN LAG(seat,  s.lng*s.lagb::int)            OVER(ORDER BY row_id DESC, seat_id     )='#' THEN 1 ELSE 0 END)
            +(CASE WHEN LAG(seat,((s.lng*s.lagbr)+s.lagbr)::int) OVER(ORDER BY row_id DESC, seat_id DESC)='#' THEN 1 ELSE 0 END)
            +(CASE WHEN LAG(seat,        s.lagl::int)            OVER(ORDER BY row_id     , seat_id     )='#' THEN 1 ELSE 0 END)
            +(CASE WHEN LAG(seat,        s.lagr::int)            OVER(ORDER BY row_id     , seat_id DESC)='#' THEN 1 ELSE 0 END)
            +(CASE WHEN LAG(seat,((s.lng*s.lagfl)+s.lagfl)::int) OVER(ORDER BY row_id     , seat_id     )='#' THEN 1 ELSE 0 END)
            +(CASE WHEN LAG(seat,  s.lng*s.lagf::int)            OVER(ORDER BY row_id     , seat_id     )='#' THEN 1 ELSE 0 END)
            +(CASE WHEN LAG(seat,((s.lng*s.lagfr)+s.lagfr)::int) OVER(ORDER BY row_id     , seat_id DESC)='#' THEN 1 ELSE 0 END)
            ELSE -1 END as occupied,
            s.lng
       FROM seats s
      WHERE counter<190

)
select count(*) as nbr,counter,sum(occupied) from seats where seat='#' group by 2 order by 2 desc limit 1;
