...
WITH recursive move_boat AS (
    ...
    SELECT m.move,
           ...
           m.move_id,
           CASE
             WHEN m.move='L' THEN (b.facing - m.len + 720) % 360
             WHEN m.move='R' THEN (b.facing + m.len) % 360
             ELSE b.facing
           END as facing,
           CASE
             WHEN m.move='E' THEN b.posx+m.len
             WHEN m.move='W' THEN b.posx-m.len
             WHEN (m.move='F' AND b.facing=90) THEN b.posx+m.len
             WHEN (m.move='F' AND b.facing=270) THEN b.posx-m.len
             ELSE b.posx
           END as posx,
           CASE
             WHEN m.move='N' THEN b.posy+m.len
             WHEN m.move='S' THEN b.posy-m.len
             WHEN (m.move='F' AND b.facing=0) THEN b.posy+m.len
             WHEN (m.move='F' AND b.facing=180) THEN b.posy-m.len
             ELSE b.posy
           END as posy
