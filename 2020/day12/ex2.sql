...
WITH recursive move_boat AS (
    ...
    SELECT m.move,
           ...
           m.move_id,
           CASE
             WHEN m.move='F' THEN b.posx+(b.wpx*m.len)
             ELSE b.posx
           END as posx,
           CASE
             WHEN m.move='F' THEN b.posy+(b.wpy*m.len)
             ELSE b.posy
           END as posy,
           CASE
             WHEN m.move='E' THEN b.wpx+m.len
             WHEN m.move='W' THEN b.wpx-m.len
             WHEN (m.move='L' AND m.len=90) OR (m.move='R' AND m.len=270) THEN -1*b.wpy
             WHEN (m.move='L' AND m.len=180) OR (m.move='R' AND m.len=180) THEN -1*b.wpx
             WHEN (m.move='L' AND m.len=270) OR (m.move='R' AND m.len=90) THEN b.wpy

             ELSE b.wpx
           END as wpx,
           CASE
             WHEN m.move='N' THEN b.wpy+m.len
             WHEN m.move='S' THEN b.wpy-m.len
             WHEN (m.move='L' AND m.len=90) OR (m.move='R' AND m.len=270) THEN b.wpx
             WHEN (m.move='L' AND m.len=180) OR (m.move='R' AND m.len=180) THEN -1*b.wpy
             WHEN (m.move='L' AND m.len=270) OR (m.move='R' AND m.len=90) THEN -1*b.wpx

             ELSE b.wpy
           END as wpy

      FROM movements m
      JOIN move_boat b ON b.move_id+1=m.move_id
)
select abs(posx)+abs(posy) FROM move_boat order by move_id DESC limit 1;
rollback;
