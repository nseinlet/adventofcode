...
WITH RECURSIVE binary_address AS(
    SELECT DISTINCT address as id,
           address::bigint%(2^36)::bigint as reminder,
           36 as position,
           address::bigint/(2^36)::bigint as value
      FROM vals_with_mask
...
tmp_masks AS (
    SELECT id,
           regexp_split_to_table(reverse(value),'') as val
      FROM vals_with_mask
      WHERE type='mask'
 ),
 all_masks AS (
 ...
 apply_mask_on_addr AS (
  SELECT vm.address,
         vm.mask_id,
         ba.position,
         ba.value,
         am.val as mask_val
    FROM vals_with_mask vm
    JOIN binary_address ba ON vm.address=ba.id
    JOIN all_masks am ON am.id=vm.mask_id AND am.position-1=ba.position
   WHERE vm.type='mem'
GROUP BY 1,2,3,4,5
ORDER BY 1,2,3
),
masks_combinations AS (
    SELECT address,
           mask_id,
           position,
           value::varchar
      FROM apply_mask_on_addr
     WHERE mask_val='0'
...
SELECT *
  INTO TEMPORARY TABLE masks_combinations
  FROM masks_combinations;

WITH RECURSIVE apply_masks AS (
    SELECT address,
           mask_id,
           value::varchar,
           position
    ...
    SELECT am.address,
           ...
           mc.position
      FROM apply_masks am
      JOIN masks_combinations mc ON mc.position=am.position-1 AND am.mask_id=mc.mask_id AND am.address=mc.address
),
combinations AS (
    SELECT address,
           mask_id,
           regexp_split_to_table(reverse(value),'') as value,
           row_number() over() as combination_id
      FROM apply_masks
     WHERE position=0
),
combinations_to_recombine AS (
    SELECT address,
           mask_id,
           value,
           combination_id,
           row_number() over(PARTITION BY combination_id)-1 as expo
      FROM combinations
),
all_combinations AS(
    SELECT address,
           mask_id,
           combination_id,
           sum(value::int*(2^(expo))) as result_addr
      FROM combinations_to_recombine
      GROUP BY 1,2,3
)
SELECT vwm.id as order,
       vwm.value::bigint,
       ac.result_addr
  INTO TEMPORARYTABLE final_combinations
  FROM vals_with_mask vwm
  JOIN all_combinations ac ON ac.address=vwm.address AND ac.mask_id=vwm.mask_id
 WHERE vwm.type='mem';
...
SELECT sum(c.value)
  ...
  JOIN final_combinations c ON r.last_order=c."order" AND c.result_addr=r.result_addr;
