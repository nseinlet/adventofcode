...
drawn as (
    SELECT regexp_split_to_table(data, ',') as num
      FROM input
     WHERE id=1
),
drawn_order as (
...
grids_raw as (
    SELECT regexp_split_to_table(data, ' ') as num,
           CASE
             WHEN MOD(id-1, 5)=0 THEN 5
             ELSE MOD(id-1, 5)
           END as row,
           (id-2)::int/5 as grid_num
    FROM input
    WHERE id>1
),
grids as (
    SELECT num,
           row,
           row_number() over(PARTITION BY grid_num,row) as col,
           grid_num
      FROM grids_raw
     WHERE TRIM(num)!=''
),
game_row as (
  SELECT g.grid_num,
         ...
         max(d.sort) as turn
    FROM grids g
    JOIN drawn_order d ON g.num=d.num
GROUP BY 1,2
),
game_col as (
  SELECT g.grid_num,
...
select sum(g.num::int*wn.num::bigint)
  from grids g
  JOIN win_game w USING (grid_num)
  JOIN LATERAL (SELECT num FROM drawn_order WHERE sort=w.turn) as wn ON TRUE
 WHERE NOT EXISTS (SELECT 1 FROM drawn_order d WHERE d.num=g.num AND d.sort<=w.turn)
