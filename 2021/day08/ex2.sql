...
one AS (
    SELECT signal,
           id
      FROM signal
     WHERE length(trim(signal))=2
),
four AS (
    SELECT signal,
           id
      FROM signal
     WHERE length(trim(signal))=4
),
seven AS (
    SELECT signal,
           id
      FROM signal
     WHERE length(trim(signal))=3
),
height AS (
    SELECT signal,
           id
      FROM signal
     WHERE length(trim(signal))=7
),
six AS (
    SELECT signal,
           missing,
           id
      FROM signal
     WHERE length(trim(signal))=6
     AND EXISTS (SELECT 1 FROM one WHERE one.id=signal.id AND one.signal like '%'||signal.missing||'%' )
),
five AS (
    SELECT signal,
           missing,
           id
      FROM signal
     WHERE length(trim(signal))=5
     AND NOT EXISTS (SELECT 1 FROM six WHERE six.id=signal.id AND signal.signal like '%'||six.missing||'%' )
),
nine AS (
    SELECT signal,
           missing,
      ...
      FROM signal
     WHERE length(trim(signal))=6
     AND EXISTS (SELECT 1 FROM five WHERE five.id=signal.id AND translate(five.missing, signal.missing, '')!=five.missing )
     AND NOT EXISTS (SELECT 1 FROM six WHERE six.id=signal.id AND six.signal=signal.signal)
),
zero AS (
    SELECT signal,
           missing,
     ...
     WHERE length(trim(signal))=6
     AND NOT EXISTS (SELECT 1 FROM six WHERE six.id=signal.id AND six.signal=signal.signal)
     AND NOT EXISTS (SELECT 1 FROM nine WHERE nine.id=signal.id AND nine.signal=signal.signal)
),
three AS (
    SELECT signal,
           missing,
      ...
      FROM signal
     WHERE length(trim(signal))=5
     AND EXISTS (SELECT 1 FROM seven WHERE seven.id=signal.id AND length(translate(signal.signal, seven.signal, ''))=2 )
),
two AS (
    SELECT signal,
           missing,
      ...
      FROM signal
     WHERE length(trim(signal))=5
     AND NOT EXISTS (SELECT 1 FROM three WHERE three.id=signal.id AND three.signal=signal.signal)
     AND NOT EXISTS (SELECT 1 FROM five WHERE five.id=signal.id AND five.signal=signal.signal)
),
translation AS (
          SELECT id, signal, 0 as nbr FROM zero
UNION ALL SELECT id, signal, 1 as nbr FROM one
UNION ALL SELECT id, signal, 2 as nbr FROM two
UNION ALL SELECT id, signal, 3 as nbr FROM three
UNION ALL SELECT id, signal, 4 as nbr FROM four
UNION ALL SELECT id, signal, 5 as nbr FROM five
UNION ALL SELECT id, signal, 6 as nbr FROM six
UNION ALL SELECT id, signal, 7 as nbr FROM seven
UNION ALL SELECT id, signal, 8 as nbr FROM height
UNION ALL SELECT id, signal, 9 as nbr FROM nine
)

SELECT sum(t1.nbr*1000+t2.nbr*100+t3.nbr*10+t4.nbr)
  FROM output o
  JOIN translation t1 ON o.id=t1.id AND length(o.o1)=length(t1.signal) AND translate(o.o1, t1.signal, '')=''
  JOIN translation t2 ON o.id=t2.id AND length(o.o2)=length(t2.signal) AND translate(o.o2, t2.signal, '')=''
  JOIN translation t3 ON o.id=t3.id AND length(o.o3)=length(t3.signal) AND translate(o.o3, t3.signal, '')=''
  JOIN translation t4 ON o.id=t4.id AND length(o.o4)=length(t4.signal) AND translate(o.o4, t4.signal, '')=''
