...
UNION ALL
    ...
    SELECT cw.awp || ',' || w.ep,
           ...
           END,
           CASE
            WHEN NOT w.epb AND array_position(cw.wp, w.ep) IS NOT NULL THEN w.ep
            ELSE cw.double_small
           END,
           w.ep
      FROM compute_ways cw
      JOIN ways w ON w.sp=cw.last_ep
     WHERE (w.epb=TRUE OR array_position(cw.wp, w.ep) IS NULL OR double_small IS NULL)
       and w.sp!='end'
       and w.ep!='start'
)
SELECT count(*)
