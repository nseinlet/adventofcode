...
inputs50 AS (
    SELECT ope,
           i.id,
           greatest(xmin, -50) as xmin,
           least(xmax,  50) as xmax,
           greatest(ymin, -50) as ymin,
           least(ymax,  50) as ymax,
           greatest(zmin, -50) as zmin,
           least(zmax,  50) as zmax
      FROM inputs i
     WHERE xmin<=50 AND xmax>=-50
       AND ymin<=50 AND ymax>=-50
       AND zmin<=50 AND zmax>=-50
       AND NOT EXISTS (SELECT 1 FROM overlapped o WHERE i.id=o.id)
),

--We'll cut space dims in ~20 slices, in every directions
--in a loop to find cubes as small as possible
cut_space AS (
    SELECT 100000 AS slice_size,
           0 AS nbr

  UNION ALL
   (WITH RECURSIVE tmp_cut AS (
       SELECT slice_size/10 as slice_size,
              nbr
         FROM cut_space
        WHERE slice_size>100000
   ),

   -- cube containing only ON lights, and no other cube contain them
   allon AS (
      SELECT i.id,
             (1+i.xmax-i.xmin)*(1+i.zmax-i.zmin)*(1+i.zmax-i.zmin) as nbr
        FROM inputs i
   LEFT JOIN inputs i2 ON i2.xmin<=i.xmax AND i2.xmax>=i.xmin
                      AND i2.ymin<=i.ymax AND i2.ymax>=i.ymin
                      AND i2.zmin<=i.zmax AND i2.zmax>=i.zmin
                      AND i2.id>i.id
      WHERE i.ope='on'
        AND i2.id IS NULL
  ),
  limits AS (
      SELECT min(xmin) as bxm,
             max(xmax) as bxp,
             min(ymin) as bym,
             max(ymax) as byp,
             min(zmin) as bzm,
  ...
  inputsall AS (
      SELECT ope,
             id,
             greatest(xmin, bxm) as xmin,
             least(xmax, bxp) as xmax,
             greatest(ymin, bym) as ymin,
             least(ymax, byp) as ymax,
             greatest(zmin, bzm) as zmin,
             least(zmax, bzp) as zmax
        FROM inputs i, limits
       WHERE NOT EXISTS (SELECT 1 FROM overlapped o WHERE i.id=o.id)
         AND NOT EXISTS (SELECT 1 FROM allon o WHERE i.id=o.id)
  ),
  slices AS (
      SELECT x.x*10000 as xmin,
             ((x.x+1)*10000)-1 as xmax,
             y.y*10000 as ymin,
             ((y.y+1)*10000)-1 as ymax,
             z.z*10000 as zmin,
             ((z.z+1)*10000)-1 as zmax,
             row_number() over() as slice_id
        FROM generate_series(-10,9) as x,
             generate_series(-10,9) as y,
             generate_series(-10,9) as z
  ),
  itmp AS (
      SELECT ope,
             i.id,
             s.slice_id,
             greatest(i.xmin, s.xmin) as xmin,
             least(i.xmax,  s.xmax) as xmax,
             greatest(i.ymin, s.ymin) as ymin,
             least(i.ymax,  s.ymax) as ymax,
             greatest(i.zmin, s.zmin) as zmin,
             least(i.zmax,  s.zmax) as zmax
        FROM inputsall i, slices s
       WHERE i.xmin<=s.xmax AND i.xmax>=s.xmin
         AND i.ymin<=s.ymax AND i.ymax>=s.ymin
         AND i.zmin<=s.zmax AND i.zmax>=s.zmin
  ),
  slices_to_process AS (
      SELECT i.slice_id,
             count(DISTINCT ope) as o_nbr,
             count(*) as nbr,
             min(ope) as ope
        FROM itmp i
        group by slice_id
  ),
  --slices that contains only one cube, which is on
  allon_slices AS (
      SELECT (1+xmax-xmin)*(1+zmax-zmin)*(1+zmax-zmin) as nbr
        FROM slices_to_process s
        JOIN itmp i ON i.slice_id=s.slice_id
       WHERE s.o_nbr=1
         AND s.ope='on'
         and s.nbr=1
  ),
  -- slices
  slices_to_compute AS (
      SELECT i.slice_id,
             row_number() over() as compute_id,
             xmin,
             xmax,
             ymin,
             ymax,
             zmin,
             zmax
        FROM slices_to_process i
        JOIN LATERAL (SELECT min(xmin) as xmin, max(xmax) as xmax, min(ymin) as ymin, max(ymax) as ymax, min(zmin) as zmin, max(zmax) as zmax FROM itmp where itmp.slice_id=i.slice_id) as e ON TRUE
       WHERE NOT (o_nbr=1 AND ope='off')
  -- ),
  --
  -- compute AS (
  --     SELECT count(*) as nbr,
  --            compute_id
  --       FROM itmp i
  --       JOIN slices_to_compute s ON s.slice_id=i.slice_id
  --       JOIN LATERAL GENERATE_SERIES(s.xmin, s.xmax) x ON TRUE
  --       JOIN LATERAL GENERATE_SERIES(s.ymin, s.ymax) y ON TRUE
  --       JOIN LATERAL GENERATE_SERIES(s.zmin, s.zmax) z ON TRUE
  --      WHERE ope='on'
  --        AND NOT EXISTS (SELECT 1 FROM itmp i2 WHERE i2.xmin<=x.x AND i2.xmax>=x.x AND i2.ymin<=y.y AND i2.ymax>=y.y AND i2.zmin<=z.z AND i2.zmax>=z.z AND i2.id>i.id AND i2.slice_id=i.slice_id)
  --        AND s.compute_id=1
  --   GROUP BY compute_id
  )
  )-- end union all cut_space
  SELECT sum(nbr) as n FROM allon_slices
  UNION ALL
  SELECT sum(nbr) FROM allon
  -- UNION ALL
  -- SELECT sum(nbr) FROM compute
)

SELECT count(*) as nbr,
       'ex1' as ex
  FROM inputs50 i
  JOIN LATERAL GENERATE_SERIES(xmin, xmax) x ON TRUE
  JOIN LATERAL GENERATE_SERIES(ymin, ymax) y ON TRUE
  JOIN LATERAL GENERATE_SERIES(zmin, zmax) z ON TRUE
 WHERE ope='on'
 AND NOT EXISTS (SELECT 1 FROM inputs50 i2 WHERE i2.xmin<=x.x AND i2.xmax>=x.x AND i2.ymin<=y.y AND i2.ymax>=y.y AND i2.zmin<=z.z AND i2.zmax>=z.z AND i2.id>i.id)

-- union all
--
-- SELECT sum(n), 'ex2' FROM (
--
-- ) as ex2
