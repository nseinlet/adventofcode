...
string_arrays AS(
    SELECT row_number() over() as id,
        regexp_split_to_array(SUBSTRING(val, 1, length(val)/2), E'') as p1,
        regexp_split_to_array(SUBSTRING(val, 1+length(val)/2, length(val)), '') as p2
    FROM input
),
common_vals AS (
    SELECT DISTINCT s1.v1
                   ,s2.v2
                   ,s1.id
      FROM (SELECT unnest(p1) v1, id FROM string_arrays) s1
      JOIN (SELECT unnest(p2) v2, id FROM string_arrays) s2 ON s1.id = s2.id
     WHERE s1.v1 = s2.v2
)
SELECT sum(CASE
        WHEN ASCII(v1)>96 THEN ASCII(v1)-96
        ELSE ASCII(v1)-64+26
        END) as val
  FROM common_vals
