...
compute AS (
  ...
  SELECT pt || array[s_row_id*100+s_col_id]
        ,s_row_id
        ,s_col_id
        ,row_number() over() as id
        ,ways.el
    FROM compute
    JOIN ways ON ways.e_row_id = compute.row_id AND ways.e_col_id = compute.col_id
    WHERE NOT s_row_id*100+s_col_id = ANY(pt)
   )
    SELECT pt
          ,row_id
          ,col_id
      ...
      WHERE NOT EXISTS (
                        SELECT 1
                         ...
                         WHERE (c2.row_id = cp.row_id AND c2.col_id = cp.col_id and c2.id<cp.id)
                            ...
                            OR (c2.el=1 and c2.id<cp.id)
                      )
   )
)

select array_length(pt, 1)-1 from compute where el=1 order by array_length(pt, 1) asc limit 1
