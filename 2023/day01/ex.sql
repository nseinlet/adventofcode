WITH vals AS (VALUES('ninefourone1
53sevenvvqm
...
clean_words AS(
    SELECT id,
    val,
    regexp_replace(val, '[a-z]', '', 'gi') as v1,
    regexp_replace(
        regexp_replace(
            regexp_replace(
                regexp_replace(
                    regexp_replace(
                        regexp_replace(
                            regexp_replace(
                                regexp_replace(
                                    regexp_replace(
                                        regexp_replace(val, 'one', 'on1e', 'gi')
                                        ,'two', 'tw2o', 'gi')
                                    ,'three', 'thr3ee', 'gi')
                                ,'four', 'fo4ur', 'gi')
                            ,'five', 'fi5ve', 'gi')
...
SELECT sum((LEFT(v1, 1)||RIGHT(v1, 1))::int)
      ,sum((LEFT(v2, 1)||RIGHT(v2, 1))::int)
  FROM clean_words
