WITH RECURSIVE vals AS (VALUES('Game 1: 4 blue; 1 green, 2 red; 4 blue, 1 green, 6 red
...
games AS (
    SELECT split_part(split_part(val, ':', 1), 'Game ', 2)::int AS game
          ,1 as game_round
          ,1+(CHAR_LENGTH(split_part(val, ':', 2)) - CHAR_LENGTH(REPLACE(split_part(val, ':', 2), ';', ''))) AS nbr_games
          ,split_part(val, ': ', 2) AS game_data
          ,split_part(split_part(val, ': ', 2), ';', 1) AS round_data
      FROM input
    ...
    SELECT game
          ...
          ,nbr_games
          ,game_data
          ,split_part(game_data, '; ', game_round + 1) AS round_data
      FROM games
     WHERE game_round < nbr_games
),
rounds AS (
    SELECT game
          ,game_round
          ,CASE 
            WHEN split_part(split_part(round_data, ', ', 1), ' ', 2) = 'red' THEN split_part(split_part(round_data, ', ', 1), ' ', 1)::int
            WHEN split_part(split_part(round_data, ', ', 2), ' ', 2) = 'red' THEN split_part(split_part(round_data, ', ', 2), ' ', 1)::int
            WHEN split_part(split_part(round_data, ', ', 3), ' ', 2) = 'red' THEN split_part(split_part(round_data, ', ', 3), ' ', 1)::int
            ELSE 0
           END AS red
          ,CASE 
            WHEN split_part(split_part(round_data, ', ', 1), ' ', 2) = 'blue' THEN split_part(split_part(round_data, ', ', 1), ' ', 1)::int
            WHEN split_part(split_part(round_data, ', ', 2), ' ', 2) = 'blue' THEN split_part(split_part(round_data, ', ', 2), ' ', 1)::int
            WHEN split_part(split_part(round_data, ', ', 3), ' ', 2) = 'blue' THEN split_part(split_part(round_data, ', ', 3), ' ', 1)::int
            ELSE 0
           END AS blue
          ,CASE 
            WHEN split_part(split_part(round_data, ', ', 1), ' ', 2) = 'green' THEN split_part(split_part(round_data, ', ', 1), ' ', 1)::int
            WHEN split_part(split_part(round_data, ', ', 2), ' ', 2) = 'green' THEN split_part(split_part(round_data, ', ', 2), ' ', 1)::int
            WHEN split_part(split_part(round_data, ', ', 3), ' ', 2) = 'green' THEN split_part(split_part(round_data, ', ', 3), ' ', 1)::int
            ELSE 0
           END AS green
      FROM games
      ORDER BY game, game_round
),
ex1 AS (
    SELECT sum(DISTINCT game) as res1
      FROM rounds 
     WHERE game NOT IN (SELECT game 
                          from rounds 
                         WHERE red>12 
                            OR green>13 
                            OR blue>14)
...
min_cubes AS (
    SELECT game
          ,max(red) * max(green) * max(blue) as p
      FROM rounds
  GROUP BY game
),
ex2 AS (
    SELECT sum(p) as res2
      FROM min_cubes
)
SELECT * FROM ex1, ex2
