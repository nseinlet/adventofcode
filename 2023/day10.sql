WITH RECURSIVE vals AS (VALUES('F-F-JJF7.F7.F-LJ7-|77.L|.FJ7--7LFL|-|F-.|--7.F7-F-.F|-FL7.FF7.F77-FLLJFLF--.FL--FF--7..FJ-L-LL-J..F-J77FF|-FF77FL|-|7J-7--7J-L77.FL.FL|-F7F-
...
input AS (
    SELECT regexp_split_to_array(val,'') as pipes
...
map as (
    SELECT p
          ...
          ,CASE p
            WHEN '|' THEN row_id-1
            WHEN 'L' THEN row_id-1
            WHEN 'J' THEN row_id-1
            ELSE row_id
            END AS r1
          ,CASE p
            WHEN '-' THEN col_id-1
            WHEN '7' THEN col_id-1
            WHEN 'F' THEN col_id+1
            ELSE col_id
            END AS c1
          ,CASE p
            WHEN '|' THEN row_id+1
            WHEN '7' THEN row_id+1
            WHEN 'F' THEN row_id+1
            ELSE row_id
            END AS r2
          ,CASE p
            WHEN '-' THEN col_id+1
            WHEN 'L' THEN col_id+1
            WHEN 'J' THEN col_id-1
            ELSE col_id
            END AS c2
           ,CASE P
            WHEN '|' THEN 1
            ...
            END AS hscore
      FROM input
 LEFT JOIN LATERAL unnest(pipes) WITH ORDINALITY as pipes(p, col_id) ON TRUE
),
spos AS (
    SELECT row_id
          ,col_id
...
cpt AS (
    SELECT m.row_id as r
          ,m.col_id as c
          ,m.p
          ,row_number() over() as v
          ,CASE WHEN m.r1=s.row_id AND m.c1=s.col_id THEN c2 ELSE c1 END as nc
          ,CASE WHEN m.r1=s.row_id AND m.c1=s.col_id THEN r2 ELSE r1 END as nr
          ,1 as steps
      FROM map m
      JOIN spos s ON s.row_id BETWEEN m.row_id-1 AND m.row_id+1
                 AND s.col_id BETWEEN m.col_id-1 AND m.col_id+1
                 AND (s.col_id != m.col_id OR s.row_id != m.row_id)
                 AND (s.col_id=m.col_id OR s.row_id=m.row_id)
     WHERE (m.r1=s.row_id AND m.c1=s.col_id)
        OR (m.r2=s.row_id AND m.c2=s.col_id)
    ...
    SELECT m.row_id as r
          ...
          ,m.p
          ,v
          ,CASE WHEN m.r1=c.r AND m.c1=c.c THEN c2 ELSE c1 END as nc
          ,CASE WHEN m.r1=c.r AND m.c1=c.c THEN r2 ELSE r1 END as nr
          ,steps+1 as steps
      FROM cpt c
      JOIN map m ON m.row_id=c.nr
                AND m.col_id=c.nc
    ),
    arr_cpt AS (
        select array_agg(r) as r
              ,array_agg(c) as c
          from subcpt
    )
    SELECT subcpt.r,subcpt.c,p,v,nc,nr,steps
      FROM subcpt
          ,arr_cpt
    WHERE p!='S'
    )
...
ex1 AS (
    SELECT (max(steps)/2)+1 as res1 FROM cpt
),
fence AS (
    SELECT DISTINCT r,c 
    FROM cpt
...
ex2 AS (
    SELECT count(*) as res2
    FROM map m
    LEFT JOIN fence f ON f.r=m.row_id AND f.c=m.col_id
    LEFT JOIN LATERAL (SELECT sum(hscore) hs, array_agg(p) as p
                        FROM fence f 
                        JOIN map m2 ON f.r=m2.row_id AND f.c=m2.col_id 
                        WHERE f.r>m.row_id
                        AND f.c>m.col_id
                        AND (f.r-m.row_id) = (f.c-m.col_id)
                    ) h ON TRUE
    WHERE f.r IS NULL
    AND mod(coalesce(hs, 0), 2)=1
)
select * from ex1,ex2